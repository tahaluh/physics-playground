...
int Application::run(std::unique_ptr<ApplicationLayer> layer)
{
    ...
    const bool fixedStepEnabled = config.fixedTimeStep > 0.0f;
    const int maxFixedStepsPerFrame = std::max(1, config.maxFixedStepsPerFrame);
    const float maxAccumulatedSimulationTime = fixedStepEnabled
                                                   ? (config.fixedTimeStep * static_cast<float>(maxFixedStepsPerFrame))
                                                   : 0.0f;
    ...
    while (!window->shouldClose() && !gInterruptRequested.load())
    {
        ...
        if (fixedStepEnabled && accumulator > maxAccumulatedSimulationTime)
        {
            accumulator = maxAccumulatedSimulationTime;
        }
        ...
        while (fixedStepEnabled && accumulator >= config.fixedTimeStep && fixedStepsThisFrame < maxFixedStepsPerFrame)
        {
        ...
        if (fixedStepEnabled && fixedStepsThisFrame == maxFixedStepsPerFrame && accumulator >= config.fixedTimeStep)
        {
            // Drop excess backlog so simulation cost cannot snowball indefinitely.
            accumulator = 0.0f;
        }
